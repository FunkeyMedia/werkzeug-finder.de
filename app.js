...
function toolkitRecommendations() {
  const wanted = /(werkzeugkoffer|werkzeugset|akkuschrauber|bohrschrauber|schraubendreher|zangen|steckschlüssel|bit set|wasserwaage|hammer)/i;
  return products
    .filter((product) => wanted.test(`${product.name} ${product.cat}`))
    .sort((a, b) => Number(/werkzeugkoffer|werkzeugset/i.test(b.name)) - Number(/werkzeugkoffer|werkzeugset/i.test(a.name)))
    .slice(0, 12);
}
...
function editorialImage(topic) {
  if (topic.tag === "Garten") return { src: "/assets/editorial/werkzeug-garten-hochbeet-bauen.jpg", alt: "Heimwerkerprojekt am Hochbeet im Garten" };
  if (["werkstatt-einrichten", "werkbank-planen", "werkzeug-aufbewahren"].includes(topic.slug)) return { src: "/assets/editorial/werkzeug-werkstatt-organisieren.jpg", alt: "Geordneter Arbeitsplatz in einer Heimwerkstatt" };
  if (["werkzeug-fuer-mietwohnung", "leiter-kaufen"].includes(topic.slug)) return { src: "/assets/editorial/werkzeug-mietwohnung-regal-montieren.jpg", alt: "Ein Regal wird in einer Wohnung montiert" };
  return { src: "/assets/editorial/werkzeug-grundausstattung-20260918.webp", alt: "Werkzeugkoffer, Akkuschrauber, Bohrer, Dübel und Schutzbrille auf einer Werkbank" };
}
function editorialFigure(topic) {
  const image = editorialImage(topic);
  return `<figure class="editorial-scene"><img src="${image.src}" alt="${image.alt}" width="1672" height="941" decoding="async"><figcaption>KI-generierte Illustration · keine konkrete Produktabbildung</figcaption></figure>`;
}
...
async function loadAmazonProducts(target = 500) {
  ...
  try {
    while (amazonCursor < amazonTotalCursors && amazonPool.length < target && amazonPool.length < 500) {
      ...
      if (status)
        status.innerHTML = `<b>Produkte werden geladen:</b> ${amazonPool.length} von bis zu 500 Artikeln sind bereits sichtbar.`;
    ...
    products = amazonPool.slice(0, 500);
    amazonState.loaded = amazonPool.length >= 500 || amazonCursor >= amazonTotalCursors;
